...
import { registerSubscriber } from '@/lib/api/notiApi';
...
function ComingSoonModalContent({ onClose }: Pick<ComingSoonModalProps, 'onClose'>) {
  const { t } = useLanguage();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 409 means already registered — treat as success
...
export function ComingSoonModal({ isOpen, onClose }: ComingSoonModalProps) {
  ...
  return (
    ...
    >
      ...
      <ComingSoonModalContent key={isOpen ? 'open' : 'closed'} onClose={onClose} />
    </div>
  );
}
